<?php
class Model_Fix extends CI_Model
{
	var $table = 'pengajuan_fix';
	var $column_order = array('pgjf_id','pgjf_tgl', 'pgjf_nama','pgjf_status', 'pgjf_status_ket'); //set column field database for datatable orderable
	var $column_search = array('pgjf_id','pgjf_tgl', 'pgjf_nama','pgjf_status', 'pgjf_status_ket'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	var $order = array('pgjf_tgl' => 'asc'); // default order  	private $db_sts;
	...
	{
		$this->db->from("pengajuan_fix");
		$this->db->join("status_pengajuan", "pgjf_id = sp_id_pgjf", "left");
		$this->db->where("pgjf_id", $id);
		$query = $this->db->get();

		return $query->row();
	}
	public function get_purchase_detail($id, $val, $n)
	{
		
		$fix = $this->cari_pengajuan($val[0]);
	...
	{
		$this->db->from("pengajuan_fix");
		$this->db->join("pengajuan_fix_detail", "pgjf_id = pgjfd_pgj_id", "left");
		$this->db->join("barang", "pgjfd_brg_id = brg_id", "left");
		$this->db->where("pgjf_id", $id);
		$query = $this->db->get();
